    ...
    public class GenreService
    {
        ...
        public async Task<(bool, string)> AddNewGenre(GenreBook newGenre)
        {
            try
            {
                using (var context = new QuanLiNhaSachEntities())
                {
                    var prD = await context.GenreBook.Where(p => p.DisplayName == newGenre.DisplayName && p.IsDeleted == false).FirstOrDefaultAsync();
                    if (prD == null)
                    {
        ...
        public async Task<(bool, string)> EditGenre(GenreBook selectedGenre)
        {
            ...
            try
            {
                using (var context = new QuanLiNhaSachEntities())
                {
                    bool IsExistID = await context.GenreBook.AnyAsync(p => p.DisplayName == selectedGenre.DisplayName && p.ID == selectedGenre.ID);
